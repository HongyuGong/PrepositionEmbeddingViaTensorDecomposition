...
int condition_size = 50; // condition_size: z-dimension in tensor
...
void initialize_parameters() {
    ...
    a = posix_memalign((void **)&W, 128, (2 * vocab_size + condition_size) * (vector_size + 1) * sizeof(real)); // Might perform better than malloc
    if (W == NULL) {
        fprintf(stderr, "Error allocating memory for W\n");
        exit(1);
    }
    a = posix_memalign((void **)&gradsq, 128, (2 * vocab_size + condition_size) * (vector_size + 1) * sizeof(real)); // Might perform better than malloc
	if (gradsq == NULL) {
        fprintf(stderr, "Error allocating memory for gradsq\n");
        exit(1);
    }
    for (b = 0; b < vector_size; b++) for (a = 0; a < 2 * vocab_size + condition_size; a++) W[a * vector_size + b] = (rand() / (real)RAND_MAX - 0.5) / vector_size;
    for (b = 0; b < vector_size; b++) for (a = 0; a < 2 * vocab_size + condition_size; a++) gradsq[a * vector_size + b] = 1.0; // So initial value of eta is equal to initial learning rate
    vector_size--;
}
...
void *glove_thread(void *vid) {
    long long a, b ,l1, l2, l3;
    long long id = *(long long*)vid;
    CREC cr;
    real diff, fdiff, temp1, temp2, temp3;
    ...
    real* W_updates3 = (real*)malloc(vector_size * sizeof(real));
    for (a = 0; a < lines_per_thread[id]; a++) {
        ...
        l3 = ((cr.word3 - 1LL) + vocab_size * 2) * (vector_size + 1);

        /* Calculate cost, save diff for gradients */
        diff = 0;
        //fprintf(stderr, "line 125 is good"); // DEBUG
        for (b = 0; b < vector_size; b++) diff += W[b + l1] * W[b + l2] * W[b + l3]; // dot product of word and context word vector
        //fprintf(stderr, "line 127 is good"); // DEBUG
        diff += W[vector_size + l1] + W[vector_size + l2] + W[vector_size + l3] - log(cr.val); // add separate bias for each word
	    ...
	    fprintf(stderr, "diff is %f", diff);
	    fprintf(stderr, "fdiff is %f", fdiff);
	    fprintf(stderr, "value is %f", cr.val);
        ...
        real W_updates3_sum = 0;

        for (b = 0; b < vector_size; b++) {
            // learning rate times gradient for word vectors
            //fprintf(stderr, "line 147 is good"); // DEBUG
            temp1 = fdiff * W[b + l2] * W[b + l3];
            temp2 = fdiff * W[b + l1] * W[b + l3];
            temp3 = fdiff * W[b + l1] * W[b + l2];
            //fprintf(stderr, "line 151 is good"); // DEBUG
            // adaptive updates
            W_updates1[b] = temp1 / sqrt(gradsq[b + l1]);
            W_updates2[b] = temp2 / sqrt(gradsq[b + l2]);
            W_updates3[b] = temp3 / sqrt(gradsq[b + l3]);
            //fprintf(stderr, "line 156 is good"); //DEBUG 
            W_updates1_sum += W_updates1[b];
            W_updates2_sum += W_updates2[b];
            W_updates3_sum += W_updates3[b];
            //fprintf(stderr, "line 160 is good"); //DEBUG 
            gradsq[b + l1] += temp1 * temp1;
            gradsq[b + l2] += temp2 * temp2;
            gradsq[b + l3] += temp3 * temp3;
        }
        //fprintf(stderr, "line 165 is good"); //DEBUG 
        ...
        //fprintf(stderr, "line 173 is good"); //DEBUG 
        ...
        W[vector_size + l3] -= check_nan(fdiff / sqrt(gradsq[vector_size + l3]));
        //fprintf(stderr, "line 179 is good"); //DEBUG 
        fdiff *= fdiff;
        gradsq[vector_size + l1] += fdiff;
        gradsq[vector_size + l2] += fdiff;
        gradsq[vector_size + l3] += fdiff;
        //fprintf(stderr, "line 183 is good"); //DEBUG 
...
int save_params(int nb_iter) {
    ...
    if (use_binary > 0) { // Save parameters in binary file
        ...
        for (a = 0; a < (2 * (long long)vocab_size +condition_size) * (vector_size + 1); a++) fwrite(&W[a], sizeof(real), 1,fout);
        fclose(fout);
        if (save_gradsq > 0) {
            if (nb_iter <= 0)
            ...
            for (a = 0; a < (2 * (long long)vocab_size + condition_size) * (vector_size + 1); a++) fwrite(&gradsq[a], sizeof(real), 1,fgs);
            fclose(fgs);
        }
    }
	...
	if (write_header) fprintf(fout, "%ld %d\n", vocab_size+condition_size+1, vector_size);
        for (a = 0; a < vocab_size; a++) {
            ...
            if (model == 0) { // Save all parameters (including bias)
                ...
                //for (b = 0; b < (vector_size + 1); b++) fprintf(fout," %lf", W[(vocab_size * 2 + a) * (vector_size + 1) + b]);
            }
            //fprintf(stderr, "line 262 is good"); // DEBUG
            ...
            fprintf(fout,"\n");
            //fprintf(stderr, "line 268 is good"); // DEBUG
            if (save_gradsq > 0) { // Save gradsq
                ...
                //for (b = 0; b < (vector_size + 1); b++) fprintf(fgs," %lf", gradsq[(vocab_size * 2 + a) * (vector_size + 1) + b]);
                fprintf(fgs,"\n");
            }
            //fprintf(stderr, "line 276 is good"); // DEBUG
        ...
        for (c=0; c < condition_size; c++) {
            ...
            if (model == 0) { // save all including bias
                for (b = 0; b < (vector_size + 1); b++) fprintf(fout," %lf", W[(2 * vocab_size + c) * (vector_size + 1) + b]);
            }
            else { // not saving bias
                for (b = 0; b < vector_size; b++) fprintf(fout," %lf", W[(2 * vocab_size + c) * (vector_size + 1) + b]);
            }
            fprintf(fout,"\n");
            if (save_gradsq > 0) {
                fprintf(fgs, "%d", c);
                for (b = 0; b < (vector_size + 1); b++) fprintf(fgs," %lf", gradsq[(vocab_size * 2 + c) * (vector_size + 1) + b]);
                fprintf(fgs,"\n");
            }
        }
        //fprintf(stderr, "line 290 is good"); // DEBUG


        if (use_unk_vec) {
            //fprintf(stderr, "line 281 is good"); // DEBUG
            real* unk_vec = (real*)calloc((vector_size + 1), sizeof(real));
            real* unk_context = (real*)calloc((vector_size + 1), sizeof(real));
            //real* unk_condition = (real*)calloc((vector_size + 1), sizeof(real));
            word = "<unk>";

            int num_rare_words = vocab_size < 100 ? vocab_size : 100;

            //fprintf(stderr, "line 289 is good"); // DEBUG
            for (a = vocab_size - num_rare_words; a < vocab_size; a++) {
                for (b = 0; b < (vector_size + 1); b++) {
                    ...
                    //unk_condition[b] += W[(vocab_size * 2 + a) * (vector_size + 1) + b] / num_rare_words;
                }
            }
            //fprintf(stderr, "line 297 is good"); // DEBUG

            fprintf(fout, "%s",word);
            if (model == 0) { // Save all parameters (including bias)
                for (b = 0; b < (vector_size + 1); b++) fprintf(fout," %lf", unk_vec[b]);
                for (b = 0; b < (vector_size + 1); b++) fprintf(fout," %lf", unk_context[b]);
                //for (b = 0; b < (vector_size + 1); b++) fprintf(fout," %lf", unk_condition[b]);
            }
            //fprintf(stderr, "line 305 is good"); // DEBUG
            if (model == 1) // Save only "word" vectors (without bias)
            ...
            //fprintf(stderr, "line 311 is good"); // DEBUG
            free(unk_vec);
            free(unk_context);
        }
...
int train_glove() {
    ...
    for (b = 0; b < num_iter; b++) {
        ...
        if (checkpoint_every > 0 && (b + 1) % checkpoint_every == 0) {
            ...
            //fprintf(stderr, "line 372 is good"); // DEBUG
            save_params_return_code = save_params(b+1);
            //fprintf(stderr, "line 374 is good"); // DEBUG
    ...
    //fprintf(stderr, "line 383 is good"); // DEBUG
    return save_params(0);
    //fprintf(stderr, "line 385 is good"); // DEBUG
}
