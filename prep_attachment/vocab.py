...
"""
#from data_util import getNonZeroWord
import numpy as np
...
class Vocab:
    ...
    def __init__(self, word_vec_fn, vocab_fn, selected_ind_fn, isAvg, \
        ...
        if (isAvg):
            ...
            self.leftVecs, self.num, self.dim = self.getVectorFromFile(word_vec_fn[0])
            self.rightVecs, _, _ = self.getVectorFromFile(word_vec_fn[1])
            self.vecs = self.leftVecs
        # load word vocab
    ...
    def getPrepVecs(self, prep_list):
            ...
            for prep in prep_list:
                    try:
                            prep_ind = self.zVocab.index(prep)
                            vec = self.zVecs[prep_ind]
                    except:
                            vec = [0] * self.dim
			    print prep, "not exist!"
                    prep_vecs.append(vec[:])
            print "# of prep_vecs", len(prep_vecs)
            return prep_vecs

    def getMultiVecs(self, word_list):
            vec_list = []
            for word in word_list:
                    try:
                            idx = self.vocabIndex[word]
                            vec = self.vecs[idx]
                            vec_list.append(vec[:])
                    except:
                            vec_list.append([0]*self.dim)
            return vec_list


    """
    below is for tensor prepositional sense
    """
    def getLeftMultiVecs(self, word_list):
            vec_list = []
            for word in word_list:
                    try:
                            idx = self.vocabIndex[word]
                            vec = self.leftVecs[idx]
                            vec_list.append(vec[:])
                    except:
                            vec_list.append([0]*self.dim)
            return vec_list

    def getRightMultiVecs(self, word_list):
            vec_list = []
            for word in word_list:
                    try:
                            idx = self.vocabIndex[word]
                            vec = self.rightVecs[idx]
                            vec_list.append(vec[:])
                    except:
                            vec_list.append([0]*self.dim)
            return vec_list

    def saveSingleSideEmbedding(self, save_path):
            """
            transform word embedding with 50-th dimension z-dimension vector
            """
            f = open(save_path, "w")
            print >> f, str(self.num)+" "+str(self.dim)
            print "zVecs number:", len(self.zVecs), "should be", 50
            disp_vec = self.zVecs[-1]
            for ind in range(self.num):
                    word = self.vocab[ind]
                    vec = self.leftVecs[ind]
                    new_vec = np.multiply(disp_vec, vec)
                    seq = [word] + [str(v) for v in new_vec]
		    #seq = [word] + [str(v) for v in vec]
                    print >> f, " ".join(seq)
            f.close()
            print "done saving single-side embedding..."


    def approxSenseNeighbors(self, phrase_list, embedding_path):
            """
            approximate verb-left-embeddings * prep-embedding
            """
            w_list = [phrase[0] for phrase in phrase_list]
            w_vecs = self.getLeftMultiVecs(w_list)
            p_list = [phrase[1] for phrase in phrase_list]
            p_vecs = self.getPrepVecs(p_list)
            # approx phrase sense and find neighbors
            ref_word_vectors = KeyedVectors.load_word2vec_format(embedding_path, binary=False)              
            for ind in range(len(phrase_list)):
                    wp_vec = np.multiply(w_vecs[ind], p_vecs[ind])
                    neighbor_vector = ref_word_vectors.similar_by_vector(wp_vec, topn=8)
                    print phrase_list[ind], neighbor_vector
